...
<span class="text-success"><strong><a href="../index.php" class="text-success">Home  <i class="fa fa-angle-double-right" style="font-size:20px;color:red"> </i><a href="index.php" class="text-success"> Dashboard</a> <i class="fa fa-angle-double-right" style="font-size:20px;color:red"> </i>Land Photos   </strong></span>
...
<div id="alert">
 ...
 <h4>Land Photos</h4></div>

 <hr class="bg-warning">

 <div class="  row diplay_land   " style=" max-width:99.9%; margin-left:-1px; clear:both;">
...
{ 
  ...
  $.ajax({
type: 'POST',
url: 'product_manage/fetch_view_land_photos.php',
 data:{land_code:land_code},

success: function(response) {


  $('.diplay_land').html(response);
}
});
...
$(document).on('click', '.page_up', function(){
  ...
  function functionAlert(msg, myYes) {
    ...
    window.location='edit_photo_rent_and_sell_house.php?house_code='+house_code+'&property_type='+property_type+'&file_storage_location='+file_storage_location ;
     ...
     $(document).on('click', '#delete_extended_land_photo', function(){
        
        var land_code = $(this).data('land_code');
        var file_location = $(this).data('file_location');
     $.ajax({
    type: 'POST',
    url: 'product_manage/delete_extend_land_photo.php',
    ...
    <div id = "confirm">
     <div class = "message" style="text-align: center;">
       <span  style="color:rgba(61, 203, 13, 0.935);">CONFIRMED MESSAGE</span><br>
      <span class="text-light" style="font-size:12px;">Photo Deleted !!</span><br>
       </div>
     <button class = "yes" style="color: white;">OK</button>
    </div>`;
    $('#alert').html(message);
    ...
    $(document).on('click', '#delete_extended_land_video', function(){
        
        var land_code = $(this).data('land_code');
        var file_location = $(this).data('file_location');
     $.ajax({
    type: 'POST',
    url: 'product_manage/delete_extend_land_video.php',
     data:{land_code:land_code,file_location:file_location},
    ...
    <div id = "confirm">
     <div class = "message" style="text-align: center;">
       <span  style="color:rgba(61, 203, 13, 0.935);">CONFIRMED MESSAGE</span><br>
      <span class="text-light" style="font-size:12px;">Video Deleted !!</span><br>
       </div>
     <button class = "yes" style="color: white;">OK</button>
    </div>`;
    $('#alert').html(message);
    ...
    $(document).on('click', '#add_extend_land_video', function(){
      
      var land_code = $(this).data('land_code');
      var username = $(this).data('username');
    
      window.location='add_lands_video.php?land_code='+land_code+'&username='+username;
