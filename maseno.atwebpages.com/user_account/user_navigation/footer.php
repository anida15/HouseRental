...
if ($result_apartment = mysqli_query($conn, "SELECT * from house WHERE username ='{$username}'AND property_type='Apartment'")) {
    $apartments = mysqli_num_rows( $result_apartment);
 }

 if ($result_mansionette = mysqli_query($conn, "SELECT * from house WHERE username ='{$username}'AND property_type='Mansionette'")) {
    $mansionette = mysqli_num_rows( $result_mansionette);
 }

 if ($result_bungalow = mysqli_query($conn, "SELECT * from house WHERE username ='{$username}'AND property_type='Bungalow'")) {
    $bungalow = mysqli_num_rows( $result_bungalow);
 }

 $air_bnb =$apartments + $mansionette + $bungalow;

 if ($result_land = mysqli_query($conn, "SELECT * from land WHERE username ='{$username}'")) {
