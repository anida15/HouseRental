...
$query =mysqli_query($conn,"SELECT * FROM membership_application  WHERE pending_application='0' AND status='NO' ORDER BY time_update ASC ");
...
<span class="text-success"><strong><a href="index.php" class="text-success">Home</a> <i class="fa fa-angle-double-right" style="font-size:20px;color:red"></i> Membership Denied </strong></span>
...
<div class="text-success m-4" > 
  <h6 style="text-align: center;"><u>Membership  Denied Request</u></h6>
</div>
...
<?php while($result=mysqli_fetch_assoc($query)){?>
            <tr>
                        ...
                        <td width="30%"> <button  style="font-size:small;" class="btn btn-outline-danger  "> Reject User <i class="fa fa-close" style="font-size:20px;color:red"></i>  </button> <br>  <sup style="z-index: 9; position:relative;" >
                        <?php   $date = $result['time_update'];  $newDate = date('Y-m-d H:i:s', strtotime($date. ' + 3 hours')); echo $newDate; ?></sup>
                        </td>
                        <td width="20%"> <button style="font-size:small;" class="btn btn-success Approve "   data-username="<?php echo $result['username']; ?>" >Approve </button>
                        </td>
...
$.ajax({
type: 'POST',
url: 'membership_process/approved_denied_request.php',
